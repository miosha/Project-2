...
def get_user_tweets(string):
	unique_ident = 'twitter_{}'.format(string)
	twits = []
...
statement += 'Tweets (tweet_id INTEGER, author TEXT, time_posted TIMESTAMP, tweet_text TEXT, retweets INTEGER)'
...
umsi_tweets = list(get_user_tweets('UMSI'))
...
for x in umsi_tweets:
	...
	tweetid.append(x['id'])
	timeposted.append(x['created_at'])
	tweettext.append(x['text'])
	retweetss.append(x['retweet_count'])
umsi_tweets_zip = zip(tweetid, author, timeposted, tweettext, retweetss)
for y in umsi_tweets_zip:
	cur.execute('INSERT OR IGNORE INTO Tweets VALUES (?, ?, ?, ?, ?)', (y[0], y[1], y[2], y[3], y[4]))
...
cur.execute("SELECT * FROM Tweets WHERE instr(tweet_text, 'RT')")
first_rt = cur.fetchone()[3]
