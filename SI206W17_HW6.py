...
def personal_map(function_input, list1):
    new_list1 = []
    for x in list1:
        new_list1.append(function_input(x))
    return new_list1
...
def program_productivity(programmers):
    prog_prod = programmers.num_programs/programmers.years_UM
    return prog_prod
prod_iter = iter(map(program_productivity, programmers))
...
students = [x.name for x in programmers]
names_and_productivities = list(zip(students, prod_list))
...
def more_than_5(programmers):
    if len(programmers.name) > 5:
        return programmers.name

long_names = filter(more_than_5, programmers)
...
# Define len_check
def len_check(lines_of_files):
    return (x for x in lines_of_files if (len(x)) > 40)
