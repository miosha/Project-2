...
consumer_key = "BRFiTZPeiGzQRy4aysb7AvwtA"
consumer_secret = "OxNcnJxiwDFK3HcBJmqhe4s6Ud8AIp1MGrTXwXo7Xg7r4UT2Z1"
access_token = "832043159328870404-z75SxTQntadIvGWiC1tvYBE6btWczs9"
access_token_secret = "xY5PkMjbcbHqJFYxty6L6fGTSE5m2f26TOzUwEltHfIlM"
...
def find_urls(string):
	list_url = re.findall('https?:\/\/[A-Za-z0-9]+(?:\.+[a-zA-Z0-9]{2,})+', string)
	return list_url
...
def get_umsi_data():
	umsi_list_url = []
	unique_ident = "umsi_directory_data"
	homeumsiurl = "https://www.si.umich.edu/directory?field_person_firstname_value=&field_person_lastname_value=&rid=All"
	page = "&page="
	x = 0
	if unique_ident in CACHE_DICTION:
		...
		for x in range (0,12):
			if x == 0:
				umsiurl = homeumsiurl
				r = requests.get(umsiurl, headers={'User-Agent': 'SI_CLASS'})
				umsi_list_url.append(r.text)
				CACHE_DICTION[unique_ident] = umsi_list_url
				cache_file = open(cache_filename, 'w')
				cache_file.write(json.dumps(CACHE_DICTION))
				cache_file.close()
			else:
				umsiurl = homeumsiurl.strip() + page.strip() + str(x).strip()
				r = requests.get(umsiurl, headers={'User-Agent': 'SI_CLASS'})
				umsi_list_url.append(r.text)
				CACHE_DICTION[unique_ident] = umsi_list_url
				cache_file = open(cache_filename, 'w')
...
title = []
k = 0;
for umsidata in get_umsi_data():
	soup = BeautifulSoup(umsidata[k], "html.parser")
	people = soup.find_all("h2")
	titles = soup.find_all("div", {"class":"field-item even"})
	for i in people:
		if((i & 3) == 0):
			names.append(i.text)
			i = i + 1
...
def get_five_tweets(string):
	unique_ident = 'twitter_University of Michigan'
	twits = []
	if unique_ident in CACHE_DICTION:
		if string in CACHE_DICTION[unique_ident]:
			twitter_find = CACHE_DICTION[unique_ident][string]
			for x in twitter_find['statuses']:
				twits.append(x['text'])
	else:
		twitter_find = api.search(q=string)
		CACHE_DICTION[unique_ident] = {}
		CACHE_DICTION[unique_ident][string] = twitter_find
